...
void testrevinoKpasi(Relatie r) {
	MDO d = MDO(r);
	IteratorMDO it = d.iterator();
	int n = 3;
	vector<int> chei = cheiInOrdineAleatoare(0, 3);
	...
	assert(itD.valid());
	itD.prim();
	TCheie cCautata = itD.element().first;
	TValoare vCautata = itD.element().second;

	itD.ultim();

	itD.revinoKpasi(3);
	TCheie cPrec = itD.element().first;
	TValoare vPrec = itD.element().second;
	

	assert(cPrec == cCautata);
	assert(vPrec == vCautata);
