...
void UI::tipareste_invers(VectorDinamic<Oferta> oferte) {
	cout << "Ofertele: \n";
	for (int i = oferte.getSize()-1; i >= 0; i--) {
		cout << "Denumire: " << oferte[i].get_denumire() << " Destinatie: " << oferte[i].get_destinatie()
			<< " Tip: " << oferte[i].get_tip() << " Pret: " << oferte[i].get_pret() << "\n";
	}
}
...
void UI::start() {
	...
	while (true) {
		...
		if (cmd == 0) {
			...
			int optiune;
			int cr;
			cout << "Sortarea poate fi efectuata 1 - crescator, 2 - descrescator";
			cin >> cr;
			if (cr == 1) {
				cout << "Ofertele pot fi sortate dupa: 1 - denumire, 2 - destinatie, 3 - tip+pret\n";
				cout << "Optiunea dorita de sortare este: ";
				cin >> optiune;
				if (optiune == 1) {
				...
				cin >> optiune;
				if (optiune == 1) {
					tipareste_invers(srv.sorteaza_denumire());
				}
				else if (optiune == 2) {
					tipareste_invers(srv.sorteaza_destinatie());
				}
				else if (optiune == 3) {
					tipareste_invers(srv.sorteaza_tip_pret());
				}
			}
		}
		else if (cmd == 8) {
